...
import type { ListAction, Certificates, CertificateEvent } from '~/types';

interface CeritifactesProps {
	projects: Certificates;
}

export const getStaticProps: GetStaticProps<CeritifactesProps> = async () => {
	const { default: rawProjects } = await import('~/data/certificates.json');
	const projects = (rawProjects as Array<CertificateEvent>).sort(
...
export default function TimelinePage({ projects: rawProjects }: CeritifactesProps): JSX.Element {
	...
	return (
		<Layout.Default seo={{ title: 'sam ─ projects' }}>
